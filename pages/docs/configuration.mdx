...
To configure how FTA interprets a project, you can either pass CLI arguments or create a `fta.json` file. FTA will automatically look for `fta.json` in the project root, but you can only specify this with `--config-path`.
...
| Argument        | `fta.json` configuration option? |
| --------------- | -------------------------------- |
